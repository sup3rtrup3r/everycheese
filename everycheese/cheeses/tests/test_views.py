...
from ..views import (CheeseListView, CheeseDetailView,
                     CheeseCreateView, CheeseUpdateView)
...
def test_good_cheese_list_view_expanded(rf):
    ...
    # We generate a request as if from a user accessing the cheese list views
    request = rf.get(url)
    ...
    # Pass in the request into the callable_obj to
...
def test_good_cheese_create_view(client, user):
    ...
    # Use the client to make request
...
def test_cheese_list_contains_2_cheeses(rf):
    ...
    request = rf.get(url)
    response = CheeseListView.as_view()(request)
    # Asser that the response contains both cheese names in the template
...
def test_detail_contains_cheese_data(rf, cheese):
    # cheese = CheeseFactory()
    # Make request for our new cheese
    url = reverse("cheeses:detail", kwargs={'slug': cheese.slug})
    request = rf.get(url)
    # Use the request to get the response
    callable_obj = CheeseDetailView.as_view()
